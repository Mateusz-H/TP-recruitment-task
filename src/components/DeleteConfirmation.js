...
export const DeleteConfirmation = ({ hideSelf }) => {
  const handleDelete = () => {
    hideSelf();
    store.dispatch(
      therapistsSlice.actions.removeTherapist(
        store.getState().therapists.selectedTherapist
      )
    );
  };
  return (
    <div className={deleteConfirmationMain}>
      <span className={spanHeader}>Czy na pewno chcesz usunąć terapeutę?</span>
